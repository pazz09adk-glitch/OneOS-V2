...
| 设计方案 | 设计基底已锁定为 OneOS V2（`src/resources/design-system/DESIGN.md`）；确认信息架构、交互路径、关键组件取舍和视图选型 | `rules/requirements-alignment-guide.md` · `.cursor/rules/oneos-v2-design-system.mdc` |
...
1. **需求与设计是继续工作的门禁**
   - 产品需求未确认时，禁止继续找视觉参考、拆页面结构、写设计方案或开发实现；先用简短摘要让用户确认目标用户、核心任务、范围/功能清单、内容来源和验收重点
   - **设计基底强制**：全部原型必须遵循 OneOS V2 设计规范；禁止再整理/比选 `src/themes/*` 的 `DESIGN.md` 候选
   - 设计方案未确认时，禁止继续写规格/计划或开发实现；在 V2 基底上确认信息架构、交互路径、关键组件取舍和视图选型
   - 发现目标、范围、内容来源、验收重点、信息架构、交互路径或视图选型存在多种合理选择时，立即停在对应门禁补充对齐
...
3. **原型按生产级界面处理**
 ...
 - **一个原型 = 侧栏一项**：默认同页内模拟列表/详情/视图切换；禁止默认用 `useHashPage` 拆成侧栏套娃下级页（见 `.cursor/rules/one-prototype-one-sidebar.mdc`）
4. **不要把截图当唯一真相**
