...
const APPROVAL_PLACEHOLDER_NAMES = new Set(['待定', '待指定', '待审核', '未指定', '?', '']);

function isApprovalPlaceholderName(name?: string) {
  if (!name) return true;
  return APPROVAL_PLACEHOLDER_NAMES.has(name.trim());
}
...
export const V2ApprovalProgress: React.FC<V2ApprovalProgressProps> = ({
  ...
  const getStatusBadge = (status: ApprovalNode['status']) => {
    if (disabled) {
      return {
        ...
        bg: 'var(--ln-surface-pearl, #F8FAFC)',
        border: 'var(--ln-hairline-strong, #D4D4D8)',
    ...
    switch (status) {
      ...
      default:
        return {
          label: '待审批',
          color: 'var(--ln-muted, #627D98)',
          bg: 'var(--ln-surface-pearl, #F1F5F9)',
          border: 'var(--ln-hairline-strong, #D4D4D8)',
          icon: <Clock3 style={{ width: '12px', height: '12px' }} />
        };
    }
  ...
  /** 左侧时间轴圆标：已通过/审批中/驳回实心；待审批用清晰灰底+描边，避免白底上「空心细圈」 */
  const getTimelineIconStyle = (status: ApprovalNode['status'], isCurrentActive: boolean) => {
    if (disabled) {
      return {
        background: 'var(--ln-surface-pearl, #F1F5F9)',
        border: '1.5px solid var(--ln-hairline-strong, #D4D4D8)',
        color: 'var(--ln-muted, #627D98)',
        boxShadow: 'none' as const,
      };
    }
    if (isCurrentActive || status === 'processing') {
      return {
        background: 'var(--oneos-primary, var(--ln-primary, #533AFD))',
        border: '1.5px solid var(--oneos-primary, var(--ln-primary, #533AFD))',
        color: '#FFFFFF',
        boxShadow: '0 0 0 4px rgba(83, 58, 253, 0.2)',
      };
    }
    if (status === 'approved') {
      return {
  ...
  /** 右侧卡片：非当前节点也保留可见描边/底色，避免与白面板糊成一片 */
  const getCardStyle = (status: ApprovalNode['status'], isCurrentActive: boolean) => {
    if (disabled) {
      return {
  ...
  const renderApproverRow = (
    ...
    if (!node.approver) return null;
    const unassigned = isApprovalPlaceholderName(node.approver.name);
    const displayName = unassigned ? '待指定' : node.approver.name;
    const initial = unassigned ? '' : node.approver.name.charAt(0);

    return (
      <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
        {node.approver.avatar && !unassigned ? (
          <img
          ...
          >
            {unassigned ? <User style={{ width: '12px', height: '12px' }} /> : initial}
          </div>
        )}
  ...
  return (
    ...
    >
      {nodes.map((node, idx) => {
        ...
        const timelineIcon = getTimelineIconStyle(node.status, isCurrentActive);
        const card = getCardStyle(node.status, isCurrentActive);
              ...
              >
                <div
                  style={{
                    ...
                    height: '28px',
                    borderRadius: '50%',
                    background: timelineIcon.background,
                    border: timelineIcon.border,
                    color: timelineIcon.color,
                    boxShadow: timelineIcon.boxShadow,
                    display: 'flex',
                    alignItems: 'center',
                    justifyContent: 'center',
                    fontSize: '12px',
                ...
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: '8px', flexWrap: 'wrap' }}>
                  <span
                    style={{
                      fontSize: '14px',
                      fontWeight: 700,
                      color: isCurrentActive ? 'var(--oneos-primary, var(--ln-primary, #533AFD))' : 'var(--ln-ink)'
                    }}
                  >
                    {idx + 1}. {node.title}
                  </span>
