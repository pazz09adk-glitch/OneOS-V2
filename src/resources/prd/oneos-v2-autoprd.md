...
5. **结构化工单表单页母版（定稿 · 方案 A）**：单单据深度填报且需右栏时采用「左主表单三段卡 + 右 340px 指派/SLA」；预览 `/prototypes/oneos-v2?view=form` 或展厅「PC 表单母版」。
6. **表单双布局（定稿）**：单单据填报页须二选一——**A 侧栏工作台**或 **B 横向整屏 B1**（主区 100% + PC 边距 20–24px）；生成前声明 `layout: sidebar | fullBleed`（`DESIGN.md` §4.9）。
7. **列表操作列（定稿）**：常用「编辑 / 处理（处置）」外侧展示（最多 2 个）；低频进 ⋮ 更多；统一 `OperationActions`（`DESIGN.md` §3.16）。
8. **按钮（定稿）**：统一 `V2Button`（primary / secondary / outline / ghost / danger / back）；同一操作区仅一个主按钮；主色走 CSS 变量（`DESIGN.md` §3.0）。
9. **图片上传（定稿）**：统一 `V2ImageUpload`——Web 点击+拖拽与缩略图新增；H5 拍照/相册双入口（`DESIGN.md` §3.17）。
...
| | 10. 故障处置 | `vehicle-fault-handling` | 故障处置 | 已对齐 (v1.4) | AI 微信报修助手、证据链与 SLA 告警 |
