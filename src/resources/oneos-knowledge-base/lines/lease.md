...
**闭环一句话**：客户准入评级 → 标准模板签约 → 提车应收对齐交车 → 周期账单核销 → 还车多部门费用结算归档。
...
2. **标准合同模板**：法务制式；红线、品牌/车型可见条款附件、锁定区。
3. **租赁合同**：调取模板；非标自动审批；编号唯一；合同即规则；E 签宝或线下回传闭环。
4. **提车应收**：按 15 日规则自动应收；开票；关联收款；触发运维交车。
5. **租赁业务台账**：首期至月底、二期起自然月；里程减免；安全/运维附加滚下期；关联收款。
6. **还车应结**：还车+E 签宝后汇总费用；关联收/付款；归档。
