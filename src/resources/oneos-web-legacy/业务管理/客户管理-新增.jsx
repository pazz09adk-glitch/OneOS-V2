...
// 业务管理 - 客户管理-新增（布局参照车辆租赁合同，两卡片：客户基本信息、客户开票信息）
...
const Component = function () {
	...
	var customerCode = useState('');
	var coopStatus = useState(undefined);
	var customerName = useState('');
	...
	var businessManager = useState(undefined);
	var remark = useState('');
	var taxId = useState('');
	...
	function handleCancel() {
		if (window.__customerAddBack) window.__customerAddBack();
	...
	var basicFields = React.createElement('div', null,
		React.createElement('div', { style: formRowStyle },
			React.createElement(FormItem, { label: '客户编码' },
				React.createElement(Input, { placeholder: '请输入客户编码', value: customerCode[0], onChange: function (e) { customerCode[1](e.target.value); }, style: inputStyle })
			),
		...
		React.createElement('div', { style: { marginBottom: 16 } },
			React.createElement(Breadcrumb, {
				items: [
					{ title: '业务管理' },
					{ title: '客户管理' },
					{ title: '新增客户', className: 'breadcrumb-last' }
				]
			})
		),
