...
// 车辆管理系统 - 转正式合同模块（布局与合同续签一致，合同编码后增加转正式合同自：合同编号xxxxxxx，IE11+ 兼容，采用 antd）
...
const Component = function() {
  ...
  var csContractOriginal = React.useState([
    { name: '租赁合同-试用版原件.pdf', size: '1.2 MB', uploadTime: '2026-02-16 14:30', isOriginal: true }
  ]);
  ...
  var contractCodeDisplay = contractCode + '（转正式合同自：合同编号' + originalContractCode + '）';
  ...
  var requirementContent = `车辆租赁合同-转正式合同（2026年3月3日版本）
「数字化资产ONE-OS运管平台」中的「车辆租赁合同」-「转正式合同」模块，在车辆租赁合同操作列点击「转正式合同」进行操作；
...
2.1.客户名称：从原合同自动反写，不可编辑，如原客户信息发生改变则按照新信息进行反写；必选项，选择器，支持从输入框内输入内容进行模糊搜索，从客户信息列表中选择对应客户（只显示已通过审核的客户）；
...
3.2.合同类型：自动变更为正式合同，必选项，选择器，合同类型分为「正式合同」「试用合同」；
