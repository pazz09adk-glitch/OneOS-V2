...
3. **三视角模式原生统一 (Three View Perspectives)**：复杂业务台账与管理页面统一提供**【1. 列表模式】**（高密度台账与筛选，可含**主表+嵌套子表**）、**【2. 看板模式】**（阶段 Pipeline 管线）、**【3. 主从工作台 (Split)**】（左任务单据列表 + 右侧工作台）。**禁止**把「列表内嵌套子表」误称为「主从/表单模式」——二者术语见 `src/resources/design-system/DESIGN.md` §5.0。
...
#### 2.4.3 13项高阶筛选网格 (FilterBar Grid)
...
- **折叠规则**：PC 默认首行展示 4 项，点击【更多筛选】展开完整 13 项；H5 模式下支持一键折叠/展开为单列表单列。
...
> **权威全文以** `src/resources/design-system/DESIGN.md` **§5 为准**（v2.8）。本副本摘要如下。

页面顶部固定 **`V2SegmentedControl` 三视角**：**列表** / **看板** / **主从工作台**。顶栏仅「三视角 + 主操作」。

### 5.0 术语门禁

- **列表 · 主表+嵌套子表** ≠ **主从工作台 (Split)**。前者是表格展开行；后者是左 320–340 列表 + 右工作台。
- 账单「合同主表 + 期账单子表」落在**列表模式**，且仍须提供看板与 Split。

### 5.1–5.3 摘要

1. **列表**：Bento KPI + Pill Tabs（count）+ 筛选连体 + 可嵌套子表 + 分页贴壳；子表底色用 Token，操作列 `OperationActions`。
2. **看板**：通常 4 列，列名按域（账单示例：待收款 / 部分收款 / 逾期 / 已结清）。
3. **主从工作台**：左可搜列表 + 右深度工作台。
