...
| ③ 设计方案对齐 | 产品需求确认后，设计基底固定为 OneOS V2（禁止再比选 themes）；在 V2 上收敛布局、交互、视图选型与内容呈现 | `rules/requirements-alignment-guide.md` · `.cursor/rules/oneos-v2-design-system.mdc` |
...
1. **产品需求和设计方案分阶段对齐**
   ...
   | 设计方案 | 出现不同信息架构、交互路径或视图选型（设计基底已锁定为 V2，不再比选） |
...
6. **一个原型 = 侧栏一项**
   - 默认同页内模拟列表/详情/视图切换；禁止默认用 `useHashPage` 拆成侧栏套娃下级页（见 `.cursor/rules/one-prototype-one-sidebar.mdc`）
7. **讲人话，用户不懂技术**
